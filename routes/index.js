...
router.post("/register", function (req, res) {
    ...
    if (req.body.adminCode === "iDoNotWantToBeAnAdmin!") {
    ...
    async.waterfall([
        ...
        function (token, user, done) {
            var smtpTransport = nodemailer.createTransport({
                service: 'Gmail',
                auth: {
                    user: 'helloworldpracticeprogramming@gmail.com',
                    pass: process.env.GMAILPW
                }
            });
            var mailOptions = {
                to: user.email,
                from: 'helloworldpracticeprogramming@gmail.com',
...
router.get("/users/:id", function (req, res) {
    User.findById(req.params.id, function (err, foundUser) {
        if (err || !foundUser) {
            req.flash("error", "Cannot find the user with that id");
            res.redirect("/campgrounds");
        } else {
            Campground.find().where('author.id').equals(foundUser._id).exec(function (err, foundCampgrounds) {
                if (err) {
                    ...
                    if (req.user && req.user._id.equals(foundUser._id)) {
                        res.render("users/show", { user: foundUser, campgrounds: foundCampgrounds, page: "profile" });
                    } else {
